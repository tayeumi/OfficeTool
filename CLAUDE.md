...
3. **Process** — each domain's `XProcessor` (a BullMQ `WorkerHost`) switches on `job.name` to a private handler. PDF tools use `pdf-lib`/`pdf-to-img`/`archiver`. Office tools shell out to LibreOffice headless via `api/src/office/libreoffice.util.ts`'s `convertWithLibreOffice(inputPath, outputDir, targetFormat, { inFilter?, timeoutMs? })` — each call gets its own temp `-env:UserInstallation` profile dir to avoid known LibreOffice headless concurrency issues; **`pdf-to-word` requires `inFilter: 'writer_pdf_import'`** or LibreOffice fails to recognize the PDF as an importable document (surfaces as a vague "no export filter" or file-write error otherwise) — `office/excel-merge` instead uses pure-JS `exceljs` (values-only copy, no styles/merged-cells). Image tools use `sharp`. **OCR** (`ocr` domain) has exactly one tool — `pdfToWordAi` — after the Tesseract/PaddleOCR-based tools (`image-to-text`, `pdf-to-text`, plain-text `pdf-to-word`) were removed (2026-09-24, they produced results judged not good enough). It uses a Vision LLM instead of a traditional OCR engine: each PDF page is rendered to a PNG (`pdf-to-img`), sent to whichever AI model the caller selected (see the `ai-models`/`ai-providers` modules below), and the model returns HTML (not plain text) — headings, tables with `colspan`/`rowspan`, bold/italic/underline, alignment, relative font size (`data-size`), first-line indent (`data-indent`), and image bounding boxes (`data-bbox`, as a percentage of page width/height) are all encoded in that HTML per a shared prompt (`api/src/ai-providers/ocr-prompt.ts`). `api/src/ocr/html-to-docx.util.ts` parses that HTML with `cheerio` into real `docx` `Paragraph`/`Table` nodes — merged table cells need an explicit `TableCell` with `verticalMerge: CONTINUE` on every row a `rowSpan` covers (docx's own `rowSpan` option only marks the origin cell, it does not synthesize the continuation cells OOXML requires), and any `<img data-bbox>` is turned into a real embedded image by cropping the box out of the *original* page PNG with `sharp` and wrapping it in an `ImageRun` — the model never draws the image itself, only reports where it is. This still isn't pixel-perfect (bbox coordinates are the model's guess, and there's no support for exact font family/color/line-spacing/headers-footers), so don't oversell it as an exact reproduction.

**AI model configuration** (`ai-models` domain, unrelated to the 4 file-processing domains' job-queue shape) lets an admin register one or more AI models (Gemini/Anthropic/OpenAI/OpenRouter, the last being an OpenAI-SDK-compatible gateway that reuses `ai-providers/openai.provider.ts` with a different `baseURL`) — `AiModelsService` persists them as plain JSON in `STORAGE_DIR/ai-models.json` (deliberately not Redis, which is TTL/cache-oriented, or a new DB, which would be overkill for a few dozen rows); `POST/PUT` never round-trip the real API key back to the frontend, only a masked last-4-chars version. **There is no admin-only auth on `/ai-models` yet** — any authenticated ams user can currently read/change/delete these shared API keys, which is a known gap, not an oversight; do not treat the current `X-User-Name` header (frontend-set, unverified) as a substitute for real auth if asked to gate this later.
